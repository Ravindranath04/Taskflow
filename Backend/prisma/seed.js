...
async function main() {
  ...
  const tasks = await Promise.all([
    ...
    prisma.task.create({ data: { title:"AI chat integration",    description:"Connect Gemini API to tasks",   status:"IN_PROGRESS", priority:"CRITICAL", assigneeId:arjun.id, creatorId:kavya.id, projectId:p1.id, tags:["ai","backend"],   requiredSkills:["Node.js","REST APIs"],      requiredDomains:["backend","api"] }}),
